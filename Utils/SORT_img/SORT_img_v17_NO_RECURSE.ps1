...
.DESCRIPTION
    ...
    3. Сканирование медиатеки с индикацией:
       - Происходит поиск файлов только в исходной папке (без вложенных папок). 
         ...
         Посторонние файлы и вложенные папки полностью игнорируются.
...
if (-not $resumeSession) {
    ...
    # PowerShell быстро находит файлы в корневой папке (без -Recurse)
    $files = @(Get-ChildItem -Path $AbsoluteSourceDir -File | Where-Object { 
